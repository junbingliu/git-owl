...
Login.prototype = {
    ...
    postData: function () {
        ...
        var shopId = self.$root.find("#shopId").val();
    ...
    check: function () {
        ...
        var checkIsAdminResult = $.ajax({
            type: "POST",
            url: "/" + self.appId + "/pages/login/checkShopAdmin.jsx",
            data: postData,
            async: false
        }).responseText;
    ...
    submit: function () {
        ...
        var postData = self.postData();
        $.post("/" + self.appId + "/pages/login/shopLoginOk.jsx", postData, function (ret) {
            if (ret.code == "0") {
                window.location.href = "/" + self.appId + "/pages/index.jsx?m=head_merchant&s=" + postData.shopId;
            } else {
