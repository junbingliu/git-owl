                ...
                $.post(that.saveSelectedPaymentUrl,{'selectedPaymentId':selectedPaymentId,mode:'of'},function(data){
                    if(data.state==="ok"){
                        ...
                        if(selectedPayInterfaceId == 'payi_0'){
                            depositAndPointsEditor = new $.DepositAndPointEditor($("#depositAndPoint"), $("#depositShow"), $("#depositEdit"), '/shopping/handle/new/setUseDepositAndPoints.jsp', window.oc, merchantId, cartType);
                            depositAndPointsEditor.show();
                            $("#depositAndPoint").find("a").trigger("click");
                            $("#depositAndPoint").find(".btnSave").find("a").trigger("click");
                        } else{
